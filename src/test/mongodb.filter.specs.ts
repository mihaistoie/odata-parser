...
import { $filter2mongoFilter } from '../index';

describe('Odata $filter to mongo Filter', function () {
    describe('#operators()', function () {
        it('Different from (not equal)', function () {
            let mf = $filter2mongoFilter('VAT_Bus_Posting_Group ne \'EXPORT\'');
            assert.deepEqual(mf, { VAT_Bus_Posting_Group: { $ne: 'EXPORT' } });

        });
        it('/', function () {
            let mf = $filter2mongoFilter('Address/city eq \'Paris\'');
            assert.deepEqual(mf, { 'Address.city': 'Paris' });
        });
        it('And', function () {
            let mf = $filter2mongoFilter('Country_Region_Code eq \'ES\' and Payment_Terms_Code eq \'14 DAYS\'');
        ...
        it('OR', function () {
            let mf = $filter2mongoFilter('Country_Region_Code eq \'ES\' or Country_Region_Code eq \'US\' or Country_Region_Code eq \'FR\'');
        ...
        it('Less than', function () {
            let mf = $filter2mongoFilter('Entry_No lt 610');
            assert.deepEqual(mf, { Entry_No: { $lt: 610 } });
        });

        it('Greater than', function () {
            let mf = $filter2mongoFilter('Entry_No gt 610 and Entry_No lt 600');
        ...
        it('Greater than or equal to', function () {
            let mf = $filter2mongoFilter('Entry_No ge 610');
            assert.deepEqual(mf, { Entry_No: { $gte: 610 } });
        });

        it('In operator test1 ', function () {
            let mf = $filter2mongoFilter('Entry_No in (610, 200, 85)');
            assert.deepEqual(mf, { Entry_No: { $in: [610, 200, 85] } });
        });

        it('Less than or equal to', function () {
            let mf = $filter2mongoFilter('Entry_No le 610');
            assert.deepEqual(mf, { Entry_No: { $lte: 610 } });
        });
        it('contains', function () {
            let mf = $filter2mongoFilter('contains(CompanyName,\'Alfreds\')', null);
        ...
        it('date', function () {
            let mf1 = $filter2mongoFilter('d1 eq \'2014-03-11T00:10:01.000Z\' and d2 eq \'2015-10-01T00:10:01.000Z\'', null);
            let e1 = {
                $and: [
            ...
            let mf2 = $filter2mongoFilter('d1 eq 2014-03-11T00:00:01.000Z and d2 eq 2015-10-01T00:10:01.000Z', null);
        ...
        it('Dbname', function () {
            let mf = $filter2mongoFilter('id eq 610', null);
            assert.deepEqual(mf, { id: 610 });
        });


        it('Complex', function () {
            let mf = $filter2mongoFilter('(contains(tolower(commune), tolower(\'M\')) or contains(tolower(idcommune), tolower(\'M\'))  or contains(tolower(operation), tolower(\'M\')))');
            assert.deepEqual(mf, {
                $or: [
